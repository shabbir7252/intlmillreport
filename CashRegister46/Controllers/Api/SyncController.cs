    ...
    public class SyncController : ApiController
    {
        ...
        public bool GetSalesmans()
        {
            try
            {
                ...
                foreach (var salesman in imillSalesmans)
                {
                    if (!salesmans.Any(x => x.Sman_Cd == salesman.Sman_Cd))
                    {
            ...
            try
            {
                foreach (var rec in cashRegisters)
                {
                    if (rec.IsDeleted)
                    {
                        ...
                        var query = $"Delete From intlmill_cash_register Where CrOid = {rec.Oid}";
                    ...
                    {
                        var result = CheckExistingRecord(rec.Oid);
                        if (result)
                        {
                            var query = $"UPDATE intlmill_cash_register SET " +
                                $"trans_date_time = '{rec.TransDate}'," +
                        ...
                        {
                            var query = "INSERT INTO intlmill_cash_register " +
        ...
        private bool CheckExistingRecord(int oid)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString;
            var connection = new SqlConnection(connectionString);
            connection.Open();
            var sqlQuery = $"select * from intlmill_cash_register Where CrOid = {oid}";
            var sqlCommand = new SqlCommand(sqlQuery, connection);
