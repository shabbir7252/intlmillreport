...
namespace ImillPda.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            foreach (var rec in transactionVMs.GroupBy(x => x.TransDate).ToList())
            {
            ...
            ViewBag.DataSource = transactionVMs.OrderByDescending(x => x.TransDate).ToList();
        ...
        public ActionResult GetTransactionDetails(long entryId)
        {
            ...
            ViewBag.Comments = trans.Comments;
        ...
        public ActionResult GetWishlistDetails(long entryId)
        {
            ...
            // var wishlist = _contextPda.WishLists.Where(x => x.EntryId == entryId && x.RemainingQty > 0).Select(a => a.TransDetailOid).ToList();
            var wishlist = _contextPda.WishLists.Where(x => x.EntryId == entryId && x.RemainingQty > 0).ToList();
            var modelList = new List<RequestedItemVM>();
            // var transDetails = _contextPda.TransactionDetails.Where(x => x.Entry_Id == entryId && wishlist.Contains(x.Line_No)).ToList();
